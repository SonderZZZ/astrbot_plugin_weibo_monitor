...
- **订阅分组**：支持按会话配置独立订阅，每个群聊/会话可以只接收指定博主的推送，未配置的全局目标保持接收全部推送。
...
`subscription_mappings` 配置项允许你为不同的会话（群聊/私聊）分别指定接收哪些博主的微博推送。

### 配置格式

每行一条，格式为 `会话ID: uid或链接,uid或链接`：

```
group_abc123: 1234567890, https://weibo.com/u/9876543210
user_xyz789: 1234567890
```

### 路由规则

- **已配置订阅的会话**：只接收已指定博主的推送
- **未配置订阅的全局目标**（`target_conversation_id` 中的会话）：保持接收全部推送
- **示例**：群A 订阅了博主甲，群B 在全局目标中但未配置订阅 → 博主甲发微博，群A 和 群B 都收到；博主乙发微博，只有群B 收到

### 纯订阅分组场景

`subscription_mappings` 可以**独立使用**，不要求必须同时配置 `target_conversation_id`。例如你只想让群A 收博主甲、群B 收博主乙，各群互不干扰：

- 清空 `target_conversation_id`（不填任何全局目标）
- 配置 `subscription_mappings`：
  ```
  群A的ID: 1234567890
  群B的ID: 9876543210
  ```
- 结果：群A 只收博主 1234567890，群B 只收博主 9876543210

### 使用建议

1. 用 `/weibo_umo` 获取目标会话的 ID
2. 在 WebUI 插件设置中找到 `subscription_mappings`，填入配置
3. 用 `/weibo_status` 确认订阅分组状态
